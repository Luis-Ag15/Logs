...
class ScanLogTest(TestCase):
    ...
    def test_scan_log_creation(self):
        # Simulate scan
        response = self.client.post(reverse('view_detalles_alumno'), {'datoqr': 'ALUMNO123'})
        
        # Check response
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['id_alumno'], 'ALUMNO123')
        
        # Check if log was created
        log = ScanLog.objects.first()
        self.assertIsNotNone(log)
        self.assertEqual(log.scanner, self.user)
        self.assertEqual(log.alumno, self.alumno)
        self.assertIsNotNone(log.timestamp)

    def test_invalid_scan_does_not_log(self):
        response = self.client.post(reverse('view_detalles_alumno'), {'datoqr': 'INVALID'})
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['id_alumno'], 0)
